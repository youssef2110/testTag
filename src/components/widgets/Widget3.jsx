import { Button, Portlet, Dropdown, Widget5 } from "@blueupcode/components"
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome"
import { Component } from "react"
import * as SolidIcon from "@fortawesome/free-solid-svg-icons"

class Widget3Component extends Component {
  state = {
    data: [
      {
      ...
      <Portlet variant="primary">
        ...
        <Portlet.Body>
          {this.state.data.map((data, index) => {
            const marginClass =
              this.state.data.length - 1 > index ? "mb-2" : "mb-0"
                      ...
                      {data.detail.map((data, index) => {
                        ...
                        return (
                          <Widget5.Item key={index}>
                            <Widget5.Info>{info}</Widget5.Info>
                            {color ? (
                              <Widget5.Value className={`text-${color}`}>
                                {value}
                              </Widget5.Value>
                            ) : (
                              <Widget5.Value>{value}</Widget5.Value>
                            )}
                          </Widget5.Item>
                        )
                      })}
